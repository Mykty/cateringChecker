...
public class BreakfastFragment extends Fragment {
    ...
    public String getIdNumber(String card_number) {
        ...
        Cursor cursor = sqdb.rawQuery("SELECT * FROM "+TABLE_PERSONNEL+" WHERE card_number LIKE '"+card_number+"'", null);
        if (cursor == null || cursor.getCount() <= 0) {
            typeCollege = false;

            Cursor cursor2 = sqdb.rawQuery("SELECT * FROM "+TABLE_COLLEGE_STUDENTS+" WHERE card_number LIKE '"+card_number+"'", null);
            if (cursor2 == null || cursor2.getCount() <= 0) {
                typeLyceum = false;

                Cursor cursor3 = sqdb.rawQuery("SELECT * FROM "+TABLE_LYCEUM_STUDENTS+" WHERE card_number LIKE '"+card_number+"'", null);
    ...
    public boolean checkInList(String id_number) {
        Iterator it = breakfastList.iterator();
        while (it.hasNext()) {
    ...
    public void checkCollegeBreakfastEater(final String id_number) {
        mDatabaseRef.child("f_time").child("breakfast").child("college").child(firebaseDate).child(id_number).addListenerForSingleValueEvent(new ValueEventListener() {
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (cursor != null && cursor.getCount() > 0) {
                    ...
                    if (dataSnapshot.exists()) {
                        ...
                        mDatabaseRef.child("days").child("college").child(firebaseDate).child("breakfast").child(id_number).setValue(getNowTime());
    ...
    public void checkLyceumBreakfastEater(final String id_number) {
        mDatabaseRef.child("f_time").child("breakfast").child("lyceum").child(firebaseDate).child(id_number).addListenerForSingleValueEvent(new ValueEventListener() {
            public void onDataChange(DataSnapshot dataSnapshot) {
                String[] params = {id_number};
                SQLiteDatabase sQLiteDatabase = sqdb;
                StringBuilder sb = new StringBuilder();
                ...
                if (cursor != null && cursor.getCount() > 0) {
                    ...
                    if (dataSnapshot.exists()) {
                        desc.setTextColor(getResources().getColor(R.color.green));
                        desc.setText("Рұқсат бар");
                        mDatabaseRef.child("days").child("lyceum").child(firebaseDate).child("breakfast").child(id_number).setValue(getNowTime());
                        playSound(bonAppetitSound);
                    } else {
                        imageAccess.setImageResource(R.drawable.error_icon);
                        desc.setTextColor(getResources().getColor(R.color.red));
    ...
    public void getDayliCountEaters() {
        ...
        mDatabaseRef.child("days").child("personnel").child(firebaseDate).child("breakfast").addChildEventListener(childEventListenerPersonnel);
        mDatabaseRef.child("days").child("college").child(firebaseDate).child("breakfast").addChildEventListener(childEventListenerCollege);
        mDatabaseRef.child("days").child("lyceum").child(firebaseDate).child("breakfast").addChildEventListener(childEventListenerLyceum);
    }
        ...
        if (res != null && res.getCount() > 0) {
            ...
            while (it.hasNext()) {
                ...
                if (personnels.getId_number().equals(personnel.getId_number())) {
                    breakfastList.remove(personnels);
                    eatersAdapter.notifyDataSetChanged();
                    return;
                }
            }
